...
export interface UseBuilderStateConfig<T> {
  table: ValidTable
  initialState: T
  id?: string
}

export interface UseBuilderStateResult<T> {
  entity: T
 ...
 * Consolidates duplicate logic from useMechState, usePilotState, useCrawlerState
 * Handles loading, saving, and resetting entity state from Supabase
 */
export function useBuilderState<T extends { id: string }>(
  config: UseBuilderStateConfig<T>
): UseBuilderStateResult<T> {
